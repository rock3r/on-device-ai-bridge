...
plugins {
    ...
    alias(libs.plugins.detekt) // Static analysis
    alias(libs.plugins.kover) // Gradle Kover Plugin
    alias(libs.plugins.ktfmt) // Kotlin code formatting
}
...
dependencies {
    ...
    intellijPlatform {
        ...
        // Plugin Dependencies. Uses `platformBundledPlugins` property from the gradle.properties file for bundled
        // IntelliJ Platform plugins.
        bundledPlugins(providers.gradleProperty("platformBundledPlugins").map { it.split(',') })

        // Plugin Dependencies. Uses `platformPlugins` property from the gradle.properties file for plugin from
        // JetBrains Marketplace.
        plugins(providers.gradleProperty("platformPlugins").map { it.split(',') })

        // Module Dependencies. Uses `platformBundledModules` property from the gradle.properties file for bundled
...
intellijPlatform {
    ...
    publishing {
        ...
        // The pluginVersion is based on the SemVer (https://semver.org) and supports pre-release labels, like
        // 2.1.7-alpha.3
